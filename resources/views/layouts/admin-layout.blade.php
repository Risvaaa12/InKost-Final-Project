...
<body>
    <main class="flex bg-gray-100 font-family-karla">
        <aside class="relative hidden w-64 bg-gray-800 shadow-xl bg-sidebar sm:block">
            <div class="p-6">
                ...
                <a href="/addComunity">
                    <button
                        class="flex items-center justify-center w-full py-2 mt-5 font-semibold bg-yellow-400 rounded-lg shadow-lg cta-btn hover:shadow-xl hover:bg-teal-400">
                        <i class="mr-3 fas fa-plus"></i> New Owner
                    </button>
                </a>
                <a href="{{ route('admin.create') }}">
                    <button
                        class="flex items-center justify-center w-full py-2 mt-5 font-semibold bg-yellow-400 rounded-lg shadow-lg cta-btn hover:shadow-xl hover:bg-teal-400">
                        <i class="mr-3 fas fa-plus"></i> New Package
                    </button>
                </a>
            ...
            <nav class="pt-3 text-base font-semibold text-white">
                <a href="daftar"
                    class="flex items-center py-4 pl-6 text-white opacity75 hover:opacity-100 nav-item">
                    <i class="px-2 fa fa-home"></i>
                    Home
                </a>
                <a href="/landing"
                    class="flex items-center py-4 pl-6 text-white opacity-75 hover:opacity100 nav-item">
                    <i class="px-2 fa fa-arrow-left"></i>
                    Dashboard
                </a>
            </nav>
        </aside>
        <div class="relative flex flex-col w-full overflow-y-hidden">
            <!-- Desktop Header -->
            <header class="items-center hidden w-full px-6 py-2 bg-gray-500 sm:flex">
                <div class="w-1/2"></div>
                <div x-data="{ isOpen: false }" class="relative flex justify-end w-1/2">
                    @auth
                        <div class="pr-5 mt-3 text-white">{{ Auth::user()->name }}</div>
                    @else
                    @endauth
                    <button @click="isOpen = !isOpen"
                        class="z-10 w-12 h-12 border-4 border-gray-400 rounded-full realtive overflowhidden hover:border-gray-300 focus:border-gray-300 focus:outlinenone">
                        <img src="https://source.unsplash.com/uJ8LNVCBjFQ/400x400" class="rounded-full">
                    </button>
                    <button x-show="isOpen" @click="isOpen = false"
                        class="fixed w-full h-full cursor-default inset0"></button>
                    <div x-show="isOpen" class="absolute w-32 py-2 mt-16 bg-white rounded-lg shadow-lg">
                        <a href="#" class="block px-4 py-2 account-link hover:text-white"><i class="mr-3 fas fa-user"></i>Account</a>
                        <a href="#" class="block px-4 py-2 account-link hover:text-white"><i class="mr-3 fas fa-sticky-note"></i>Support</a>
                        <a href="#" class="block px-4 py-2 account-link hover:text-white"><i class="mr-3 fas fa-sign-out-alt"></i></i>Sign Out</a>
                    </div>
                </div>
            </header>
            <!-- Mobile Header & Nav -->
            <header x-data="{ isOpen: false }" class="w-full px-6 py-5 bg-sidebar sm:hidden">
                <div class="flex items-center justify-between">
                    <a href="#" class="text-3xl font-semibold text-white uppercase hover:textgray-300">Admin</a>
                ...
                <nav :class="isOpen ? 'flex' : 'hidden'" class="flex flex-col pt-4">
                    @auth
                        <div class="flex items-center py-2 text-white opacity-75 hover:opacity-100 pl4 nav-item"><i class="mr-3 fas fa-user"></i>{{ Auth::user()->name }}</div>
                    @else
                    @endauth
                    <a href="#"
                        class="flex items-center py-2 text-white opacity-75 hover:opacity-100 pl4 nav-item">
                        <i class="mr-3 fas fa-tachometer-alt"></i>
                        Dashboard
                    </a>
                    <a href="#" class="flex items-center py-2 text-white opacity-75 hover:opacity-100 pl4 nav-item">
                        <i class="mr-3 fas fa-sign-out-alt"></i>
                        Sign Out
                    </a>
                </nav>
            </header>
            <div class="flex flex-col w-full min-h-screen overflow-x-hidden border-t">
                <main class="flex-grow w-full p-6">
                    {{ $slot }}
                </main>
                <!-- start footer -->
                <footer class="relative px-4 py-12 mt-12 text-white bg-gray-900 sm:px-8 lg:px-16 xl:px-40 2xl:px-64 lg:py-24">
